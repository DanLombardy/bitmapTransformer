...
function prepBMP(func){
  ...
  switch(func){
  case 'Make_It_Grayscale':
    takeTransform(transform.transformList.Color_Make_It_Grayscale(getPalette()), 1078).copy(map, 54, 0);
    fs.appendFile('Grayscale.bmp', map, function (err) {
      if (err) throw err;
      console.log('The grayscale file was created');
    });
  }
}
