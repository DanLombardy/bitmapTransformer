...
function prepBMP(transformType, bitmapOrig, fileName, callback){
  var name = fileName.replace(/Color/g, "").replace(/_/g, " ");
  var map = fs.readFileSync(bitmapOrig);
  takeTransform(transformType(getPalette(map)), 1078).copy(map, 54, 0);
  fs.writeFile(name + '.bmp', map, function (err) {
    if (err) throw err;
    console.log('You\'ve created a file with the' + name + ' transformation!');
  });
}
